...
int main()
{
	...
	unsigned char man_seed[] = {0x96, 0xa3, 0x26, 0x30, 0x24, 0x30, 0x0e, 0x06, 0x03, 0x55, 0x1d, 0x0f, 0x01, 0x01, 0xff, 0x04,
								0x97, 0xf5, 0x64, 0xbc, 0x24, 0x4a, 0x7d, 0xe8, 0xa0, 0x15, 0xa4, 0xe8, 0xb6, 0x05, 0xf9, 0xa8};
	...
	mbedtls_x509write_crt_set_key_usage(&cert_man, MBEDTLS_X509_KU_DIGITAL_SIGNATURE | MBEDTLS_X509_KU_KEY_CERT_SIGN);
	...
	mbedtls_x509write_crt_set_key_usage(&cert_root, MBEDTLS_X509_KU_DIGITAL_SIGNATURE | MBEDTLS_X509_KU_KEY_CERT_SIGN);

	// The validity of the crt is specified
	ret = mbedtls_x509write_crt_set_validity(&cert_root, "20240101000000", "20250101000000");
	if (ret != 0)
	{
		return 0;
	}
	...
	ed25519_sign(kernel_signature, buffer+sm_size, payload_size-sm_size, pp_key_pub, pp_key_priv);
	int res = ed25519_verify(kernel_signature, buffer+sm_size, payload_size-sm_size, pp_key_pub);
	...
	pkey = EVP_PKEY_new_raw_private_key(EVP_PKEY_ED25519, NULL, device_root_key_priv, 32);
	if (!pkey)
	{
	...
	EVP_PKEY *pubkey = EVP_PKEY_new_raw_public_key(EVP_PKEY_ED25519, NULL, device_root_key_pub, 32);
	if (!pubkey)
	{
		ERR_print_errors_fp(stderr);
		return -1;
