...
extern byte sanctum_kernel_hash[64];
...
int main()
{
	...
	print_uart("Creating variables to interact with PUF-ROT!\r\n");
	...
	print_uart("Retrieving the ID of the platform\r\n");
	...
	print_uart("ID of the platform:\r\n");
	for (int i = 0; i < 32; i++)
		print_uart_byte(out_puf_1[i]);
	print_uart("\r\n-----------------------------------------------------------\r\n");
	...
	print_uart("Hash SM with hw function\r\n");
	...
	print_uart("SM signature verification\r\n");
	print_uart("\r\n-----------------------------------------------------------\r\n");


	if ((ed25519_verify(sd_sm_signature, sm_data_to_hash, sanctum_sm_size, pub_key_platform_provider)) == 0)
	{
		print_uart("Error in SM signature verification!\r\n");
		goto verfication_error;
	}

	print_uart("Kernel signature verification\r\n");
	print_uart("\r\n-----------------------------------------------------------\r\n");


	if ((ed25519_verify(sd_kernel_signature,((void *)DRAM_BASE)+sanctum_sm_size, sd_payload_size-sanctum_sm_size, pub_key_platform_provider)) == 0)
	{
		print_uart("Error in kernel signature verification!\r\n");
		goto verfication_error;
	}
	ed25519_create_keypair(sanctum_device_root_key_pub, sanctum_device_root_key_priv, out_puf_1);

	print_uart("Before CDI creation!\r\n");
	print_uart("\r\n-----------------------------------------------------------\r\n");
	...
	print_uart("Before ECA Keys creation!\r\n");
	print_uart("\r\n-----------------------------------------------------------\r\n");
	...
	print_uart("Before creating ECA cert DER signing with EdDSA hw function\r\n");
	print_uart("\r\n-----------------------------------------------------------\r\n");

	memcpy(sanctum_cert_sm, certDRK, dim_cert);
