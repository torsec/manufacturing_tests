...
// not used here just for test purposes
static const unsigned char sanctum_uds[] = {
    ...
    0xd8, 0x04, 0xdb, 0x13, 0xe7, 0xd7, 0x82, 0x6c, 0x70, 0x73};

static const unsigned char pub_key_manifacturer[] = {
    0x7f, 0x70, 0xec, 0x0c, 0x2a, 0x76, 0x07, 0xd3, 0x1a, 0x0b, 0x89,
    0xd6, 0x4e, 0x0f, 0x51, 0x4e, 0xf7, 0x88, 0x7d, 0x45, 0xb3, 0x2b,
    0x14, 0x8b, 0x82, 0x25, 0xd0, 0x72, 0xba, 0xf0, 0x62, 0x08};

static const unsigned char pub_key_platform_provider[] = {
    0xac, 0x56, 0x13, 0x53, 0x5c, 0x0e, 0x67, 0xf3, 0x5f, 0xb9, 0x61,
    0xf2, 0x4c, 0xc6, 0xb2, 0x81, 0x92, 0x5c, 0xf0, 0xf0, 0xc8, 0x1c,
    0x69, 0x73, 0x95, 0xd2, 0xc0, 0xcb, 0xcf, 0x94, 0x7d, 0xfe};
...
int main() {
  ...
  const unsigned char OID_algo[] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                    0x03, 0x04, 0x02, 0x0A};
  ...
  if (verify_hw(ms2xl_eddsa, pub_key_platform_provider, (void *)DRAM_BASE,
                sanctum_sm_size, sd_sm_signature, 0) == 0) {
  ...
  if (verify_hw(ms2xl_eddsa, pub_key_platform_provider,
                ((void *)DRAM_BASE) + sanctum_sm_size,
                sd_payload_size - sanctum_sm_size, sd_kernel_signature,
                0) == 0) {
  ...
  int ret =
      mbedtls_x509write_crt_set_issuer_name_mod(&cert, "CN=Root of Trust");
  if (ret != 0) {
    goto verfication_error;
  }
  ...
  ret = mbedtls_pk_parse_public_key(&issu_key, sanctum_device_root_key_priv, 64,
                                    1);
  if (ret != 0) {
    goto verfication_error;
  }

  ret = mbedtls_pk_parse_public_key(&issu_key, sanctum_device_root_key_pub, 32,
                                    0);
  if (ret != 0) {
    goto verfication_error;
  }
  ...
  if (mbedtls_x509write_crt_set_dice_tcbInfo(&cert, tcbInfo, dim, buf,
                                             sizeof(buf)) != 0)
    goto verfication_error;
  ...
  // clear all the memory locations that conains sensitive data
